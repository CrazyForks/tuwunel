...
/// Pin the pagination bounds of `/rooms/{roomId}/messages`.
///
/// Room events occupy sparse positions in a global stream, so a `to` token can
/// name a position this room has no event at. Each case drives real HTTP
/// pagination and asserts the walk stops at that boundary, excludes the
/// boundary event, and rejects a malformed token.
#[test]
...
async fn global_bounds(services: &Services, base: &str) -> Result {
	...
	for (dir, from, to) in [
		...
		assert_page(&bounded, &[middle.0.as_str()], Some(&middle.1));
	}
...
async fn backfilled_bounds(services: &Services, base: &str) -> Result {
	...
	// The events sit at -50, -30 and -10, so a bound of -40 or -20 names a
	// position no event occupies while -30 lands on one.
		...
		[("b", "0", last.0.as_str(), "-10"), ("f", "-60", first.0.as_str(), "-50")]
	{
		...
		assert_page(&bounded, &[expected], Some(end));
	}
...
/// remote server.
///
/// Both the timeline row and the event-id mapping move to the new backfilled
/// position, so the old identifier no longer resolves once this returns.
/// Callers clear the service cache before reading the events back.
async fn move_to_backfilled_position(
	...
	let pduid_pdu = services.db.get("pduid_pdu")?;
	let stored = pduid_pdu.get(&old).await?;

	pduid_pdu.insert(&new, stored.as_ref());
	pduid_pdu.remove(&old);
	services
		.db
		.get("eventid_pduid")?
